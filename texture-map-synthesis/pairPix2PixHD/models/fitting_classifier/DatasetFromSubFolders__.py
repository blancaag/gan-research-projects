...
def make_list_of_paths_labels(root, classes=None, split=None):
...
class DatasetFromSubFolders(data.Dataset):
    ...
    def __init__(self, root, data_transforms=None, label_transforms=None, loader=opencv_loader, mult_dd=False, split=0.8, mode=None):
        ...
        if mode in ['train', 'val']: 
            ...
            for i in self.classes:
                classf_content = glob(os.path.join(os.path.join(root, i), '*'))
                if any(os.path.isdir(j) for j in classf_content): mult_dd = True
            
            if mult_dd: dataset = make_list_of_paths_labels(root, self.classes, split)
