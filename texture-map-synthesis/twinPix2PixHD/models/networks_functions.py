...
def define_C(input_nc, ndf, n_layers_C, norm='instance', use_sigmoid=False, num_C=1, getIntermFeat=False, gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
    netC = MultiscaleDiscriminator(input_nc, ndf, n_layers_C, norm_layer, use_sigmoid, num_C, getIntermFeat)
    # print(netC)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        netC.cuda(gpu_ids[0])
